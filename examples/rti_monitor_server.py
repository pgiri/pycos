...
# run this program and then client (rti_monitor_client.py) either on same node or different node
...
# when client invokes RTI in this program, function below is used to start a task
def rti_server(a, b=1, task=None):
...
if __name__ == '__main__':
    ...
    # register rti_server so remote clients can request execution
    rti = pycos.RTI(rti_server)
    rti.register()
