...
# date in to memory by each (remote) server process. Remote tasks
...
# is large. See 'dispycos_client9_node.py' which uses 'node_available' and
...
def server_available(location, data_file, task=None):
    ...
    # task is executed, 'depends' of computation would've been transferred.
...
def setup_server(data_file, task=None):  # executed on remote server
    # variables declared as 'global' will be available in tasks for
...
def client_proc(computation, task=None):
    ...
    # execute 10 jobs (tasks) and get their results. Note that
