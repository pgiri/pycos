...
# This example uses status messages and message passing to run 'setup' task
# at remote process to prepare it for processing jobs.

import pycos.netpycos as pycos
from pycos.dispycos import *

# Unlike in earlier versions of pycos, computations can now take time - even
# if computations don't "yield" to scheduler, pycos can still send/receive
...
def compute_task(task=None):
    ...
    while True:
        ...
        # task (which will be received and put in this task's message
        # queue) or this task can send messages to client
        time.sleep(n)
        result += n
