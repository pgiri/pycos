...
import pycos
from pycos import _AsyncPoller, Pycos, Task
...
if platform.system() == 'Windows':
    ...
    class _AsyncFile(object):
        ...
        def __init__(self, path_handle, mode='r', share=None):
            ...
            if not _AsyncFile._notifier:
                _AsyncFile._notifier = pycos._AsyncNotifier.instance()
    ...
    class _AsyncFile(object):
        ...
        def read(self, size=0, full=False, timeout=None):
            ...
            def _read(size, full):
                ...
                try:
                    ...
                    self._read_task = self._read_data = None
                    return
                ...
                if buf:
                    ...
                    self._buflist.append(buf)
                    if full:
                        self._read_data = partial_func(_read, size, full)
                        return
                if self._buflist:
                    buf, self._buflist = b''.join(self._buflist), []
                _AsyncFile._notifier.clear(self, _AsyncPoller._Read)
        ...
        def write(self, buf, full=False, timeout=None):
            ...
            def _write(view, written):
                try:
                    ...
                    if exc.errno in (errno.EAGAIN, errno.EINTR):
                        ...
                        self._write_task = self._write_data = None
                        return
                written += n
                if n == len(view) or not full:
                    ...
                    view = view[n:]
                    self._write_data = partial_func(_write, view, written)
            ...
            self._write_task = Pycos.cur_task(self._pycos)
            self._write_task._await_()
            self._write_data = partial_func(_write, view, 0)
        ...
        def close(self):
            ...
            if self._fileno:
                ...
                self._read_task = self._write_task = None
                self._read_data = self._write_data = None
                self._buflist = []

        def _eof(self):
            """Internal use only.
            """
            if self._read_data and self._read_task:
                self._read_data()
        ...
        def _timed_out(self):
            ...
            """
            if self._read_task:
                if self._read_data and self._buflist:
                    buf, self._buflist = b''.join(self._buflist), []
                    _AsyncFile._notifier.clear(self, _AsyncPoller._Read)
                    self._read_task._proceed_(buf)
                else:
            ...
            if self._write_task:
                written = 0
                if self._write_data:
                    written = self._write_data.args[2]
