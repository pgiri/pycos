...
# 'run_jobs_proc' creates jobs as and when a dispycos process is found,
# or a process finishes a job. Each job is processed by a remote task
# (rtask) and a corresponding local task (with 'client_proc')
# interacts with that rtask to send the input data to where rtask is,
# an instance of class C to give the information needed to process the
# file. Client then waits for rtask to finish processing job, send the resulting
...
# this generator function is used to create local task (at the
# client) to communicate with a remote task
...
if __name__ == '__main__':
    ...
    Scheduler()
    # unlike in earlier examples, rtask_proc is not sent with
